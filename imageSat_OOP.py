...
class GeoProcessor:
                ...
                if "geometry" in feature and "coordinates" in feature["geometry"]:
                    ...
                    if geom_type == "Point":
                        ...
                        if len(coords) > 1 and coords[0] != coords[-1]:
                            print(
                                "Warning: LineString coordinates are not closed. Forcing closure."
                            )
    ...
    def create_json_tile(self, data_input):
        # todo : add input argument validators
        return {"type": "Polygon", "coordinates": data_input}

    def create_json_aoi(self, data_input):
        # todo : add input argument validators
        return {"type": "Point", "coordinates": data_input[0]}
    ...
    def filtered_assets(self, assets_input):
        ...
        for asset in assets_input["assets"]:
            try:
                ...
                print(f"Error when processing an asset : {e}")
                continue
        ...
        best_asset = min(
            assets, key=lambda x: (x["cloud_cover"], -x["start_time"].timestamp())
        )
        print(
            f"Best asset found : {best_asset['id']} | {best_asset['start_time']} | {best_asset['cloud_cover']}% of cloud cover"
        )
    ...
    def calculate_indices(self, pixels):
        ...
        mndwi = (pixels["B3"].astype(float) - swir) / (
            pixels["B3"].astype(float) + swir
        )
    ...
    def compute_sma(self, in_pixels):
        ...
        endmembers = np.array([[0.3, 0.4, 0.2], [0.05, 0.6, 0.1], [0.02, 0.03, 0.5]])
        rows, cols = in_pixels["B4"].shape
        new_pixels = np.stack([red.ravel(), nir.ravel(), swir.ravel()], axis=1)
        fractions = np.array(
            [nnls(endmembers.T, in_pixel)[0] for in_pixel in new_pixels]
        )
    ...
    def save_sma(self, in_fractions, outfile_tif):
        ...
        meta = {
            ...
            "transform": rasterio.transform.from_origin(
                west=0, north=10, xsize=1, ysize=1
            ),
            "width": 800,
            "height": 800,
        }
    ...
    def compute_VIF(self, in_pixels):
        ...
        ndvi = (in_pixels[nir].astype(float) - in_pixels[red].astype(float)) / (
            in_pixels[nir] + in_pixels[red]
        )
    ...
    def display_VIF(self, in_pixels, in_vif, in_ndvi, in_ndbi):
        fig, axes = plt.subplots(2, 3, figsize=(14, 14))
        ax_iter = axes.flat
        rasterio.plot.show(
            in_pixels["B4"], ax=next(ax_iter), cmap="gray", title="B4 (Red)"
        )
        rasterio.plot.show(
            in_pixels["B8"], ax=next(ax_iter), cmap="gray", title="B8 (Near infra-red)"
        )
        rasterio.plot.show(in_pixels["B11"], ax=next(ax_iter), cmap="gray", title="B11")
        ndbi = np.clip(in_ndbi, -1, 1)
        ndvi = np.clip(in_ndvi, -1, 1)
        vif = np.clip(in_vif, -1, 1)
        ...
        rasterio.plot.show(
            ...
            ax=next(ax_iter),
            cmap="gray",
            title="VIF Vegetation/Impervious Surface Fraconti",
        )
        plt.show(block=True)
        plt.show(block=True)
    ...
    def show_aoi_map(self, ASSET_ID, map_center):
        ...
        asset = response.json()
        folium_fig, folium_map = self.setup_folium(
            location=map_center, zoom=15, width=800, height=800
        )
...
def main():
    ...
    coordinates_aio, coordinates_polygon = processor.extract_coordinates(
        input_from_geoson
    )
    ...
    assert ASSET_ID
    image_content = processor.fetch_image_data(
        ASSET_ID, polygon_json, ["B2", "B3", "B4", "B5", "B6", "B7", "B8", "B11"]
    )
    geotiff, pixels = processor.process_image_data(
        image_content, ["B2", "B3", "B4", "B5", "B6", "B7", "B8", "B11"]
    )
    ndvi, ndbi, mndwi, bai, rationir_swir, ratioswir_nir = processor.calculate_indices(
        pixels
    )
    fig, axes = plt.subplots(2, 3, figsize=(14, 14))
    ax_iter = axes.flat
    ...
    rasterio.plot.show(
        rationir_swir, ax=next(ax_iter), cmap="gray", title="NIR/SWIR Ratio"
    )
    rasterio.plot.show(
        ratioswir_nir, ax=next(ax_iter), cmap="gray", title="SWIR/NIR Ratio"
    )
    plt.show()

    vif, ndvi, ndbi = processor.compute_VIF(pixels)
    processor.display_VIF(pixels, vif, ndvi, ndbi)
